...
public class UTMPoint {
	...
	public static UTMPoint latLong2UTM(double latitude, double longitude) {
		final int zoneNumber = getZoneNumber(latitude, longitude);
		...
		final char zoneLetter = getLetterDesignator(latitude);

		return new UTMPoint(zoneNumber, zoneLetter, utmEasting, latitude < 0f ? utmNorthing + FALSE_NORTHING : utmNorthing);
	}
